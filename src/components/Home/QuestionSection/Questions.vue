<template>
    <section id="questions" class="px-10 sm:px-24">
        ...
        <div class="grid md:grid-cols-2">
            <div>
                ...
                <template v-for="(question, index) in questions">
                    <QuestionItem class="relative"
                        ...
                        :class="{'!bg-active-item': index === activeIndex, 'font-bold': index===activeIndex}"
                    >
                        <div class="font-normal px-4 relative bottom-0 left-0 md:hidden bg-#FAFBFF py-4" :class="{'hidden': index !== activeIndex}">
                            {{ question.text }}
                        </div>
                    </QuestionItem>
                </template>
            ...
            <div class="bg-comments-bg-color shadow-shadow3 rounded-16px
                        px-8 md:pl-20 lg:pl-28 py-10 mt-7 md:mt-0 md:relative -left-10 -top-7 -z-1 
                        min-h-590px lg:min-h-472px hidden md:inline" >
                <h4 class="font-semibold text-lg text-question-text-color mb-10">{{ question }}</h4>
...
<script setup>
    import SectionsTitle from "@/components/Home/SectionsTitle.vue"
    import QuestionItem from "@/components/Home/QuestionSection/QuestionItem.vue"
    import {ref} from "vue";
